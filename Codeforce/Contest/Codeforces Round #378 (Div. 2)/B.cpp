...
int main()
{
	int lt, rt, bt, temp, n, mx, mxi,tlt,trt;
	pair<int, int> ary[N];
	bool flag;
	...
	while (sfi(n) == 1)
	{
		...
		loopl(0, n)
		{
			sfi2(ary[i].first, ary[i].second);
			lt += ary[i].first;
			rt += ary[i].second;
		}

		bt = mx = fabs(lt*1.0 - rt*1.0);

		flag = false;

		loopl(0, n)
		{
			tlt = lt - (ary[i].first - ary[i].second);
			trt = rt - (ary[i].second - ary[i].first);

			temp = fabs(tlt*1.0 - trt*1.0);
